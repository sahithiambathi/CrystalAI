...
class ConvLayer(nn.Module):
    ...
    def forward(self, atom_in_fea, nbr_fea, nbr_fea_idx):
        num_atoms, max_neighbors = nbr_fea_idx.shape
        atom_nbr_fea = atom_in_fea[nbr_fea_idx, :]
        total_nbr_fea = torch.cat(
            [
                atom_in_fea.unsqueeze(1).expand(num_atoms, max_neighbors, self.atom_fea_len),
                atom_nbr_fea,
                nbr_fea,
            ],
            dim=2,
        ...
        total_gated_fea = self.bn1(total_gated_fea.view(-1, self.atom_fea_len * 2)).view(
            num_atoms, max_neighbors, self.atom_fea_len * 2
        )
